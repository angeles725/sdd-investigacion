...
the full grammar and every current form, including the campaign-continuation and campaign-stop forms —
for example `next: <gap-id>` when the loop continues within the current focus.
A return without one is a silently stopped iteration. Ending with "shall I continue?" or any equivalent
question is a contract violation — the no-question rule from the triage section is a
