...
| 32 | pancaddia-leon-tunnel | `$RESEARCH_HOME/tunnel/clientes/Leon-Guanajuato/Pancaddia` | **incipient** (5 blocks @2026-09-14 / 1 run / 1 retro / nested corpus / git yes / hook no; **document-cycle** COMPLETE) `[CERT]` · incidente 2026-09-14 | Sistema de datos del JACE-9000 Pancaddia León (mini-PC + túnel `nave-panccadia` + station Niagara ATLAS 4.15.3 + Supabase): incidente pipeline caído / oBIX degradado y las soluciones (watchdog + alertas Cloudflare + reporte diario PDF+Excel) `[CERT-live]` | SSH/PowerShell a mini-PC + curl edge + Supabase SQL + Cloudflare API (todo read-only en sesión) | Spanish (runbook operativo) · technical EN markers `[CERT]` |
