...
| 13 | three.js | `$RESEARCH_HOME/prototipos/three.js/threejs-hvac-prototipos/research` | **mature** (80 md @2026-09-25 / 10 runs / 5 retros / git yes / remote yes / hook file yes / unregistered) `[CERT]` · settings.json references the hook but no session launches from research/ (#1134) | Three.js library research over local HTML prototypes (voxel/realistic HVAC files, primary `[CERT]`) + official web docs (context7 `/mrdoob/three.js`, threejs.org) + live browser probes `[CERT-hw]` | fetch-doc.sh + direct reading + context7 MCP + tools/*.mjs (Puppeteer probes, dynamic §12) | English `[CERT]` |
...
| 26 | COB-IM2 | `$RESEARCH_HOME/investigacion/COB-IM2` | **intermediate** (51 blocks @2026-09-22 / 2 runs / 1 retro / git yes / remote yes / hook yes) `[CERT]` → §26 | HVAC DWG 9-sheet | ezdxf+tools | English |
...
| 28 | sullair | `$RESEARCH_HOME/investigacion/sullair` | **intermediate** (23 md @2026-09-22 / 11-of-13 gaps / git yes / remote yes / hook yes) `[CERT]` · nested corpus under `corpus/`; remote `angeles725/investigacion-sullair` PRIVATE — detail §28 | Sullair Supervisor air-compressor controllers comms — RS-485 + Modbus protocol RE; 1 PRIMARY PDF + secondary consultant report + AI-artifact HTML, no binary `[CERT-doc]` — detail §28 | `extract-pdf.sh` + `fetch-doc.sh` + web scout | English (corpus) `[CERT]` |
...
| 30 | panccadia-3d-viewer | `$RESEARCH_HOME/panccadia-3d-viewer` | **intermediate** (38 md @2026-09-22 / 8 retros / git yes / remote no / hook file yes / unregistered) `[CERT]` · settings.json references the hook but no session launches from the target root (§30) | Standalone HTML viewer: Three.js nave 3D (landing) + DashboardPan 2D dashboard portado 1:1 `[CERT]` | none (HTML/JS; Playwright headless verify) | Spanish UI · technical EN docs `[CERT]` |
...
| 34 | fluke-177x-datos | `$RESEARCH_HOME/investigacion/fluke-177x-datos` | **incipient** (30 blocks @2026-09-22 / git yes / remote no / hook file yes / unregistered) `[CERT-live]` · settings.json references the hook but no session launches from the target root — detail §34 | Fluke 1777+1775 PQ analyzers data-extraction research: REST API (protocol 5.0.0, only GET /device open) + VNC 5900 + Energy Analyze Plus + Fluke Connect; scanned NO Modbus/BACnet/OPC-UA `[CERT-live]` | `fetch-doc.sh` + WebSearch/WebFetch | Spanish (recomendacion) · technical EN markers |
...
dropped on 2026-09-23 (#954). The kit sweeps fire only in Claude sessions opened in the kit checkout
(not from this target); from any target, run the scripts manually: `toolbelt/sweep-retros.sh`,
`toolbelt/verify-kit-clean.sh`, etc.
...
**Hook status** (corrected 2026-09-25, #1147 round 2): `.claude/hooks/retro-gate-stop.sh` IS registered as a Stop hook in `.claude/settings.json`, and the script exists and is executable. It has never fired in practice because no session in the retained transcript history (`~/.claude/projects`) was ever launched from this target's own root — every session touching this corpus ran from elsewhere (`niagara-research`, `sdd-investigacion`, `tunnel/clientes/Leon-Guanajuato/Pancaddia`, `prototipos/three.js/...`), so the registered `settings.json` was never loaded. Same standard as row 13 (three.js, #1134): registered ≠ firing.
...
### 23 — tradingview-mcp `[CERT]`

Source repository for the upstream `tradesdontlie/tradingview-mcp` Node.js MCP server and CLI. The
Research-SDD corpus is nested under `corpus/` to isolate it from the existing application source and
an approved uncommitted capability-hardening candidate. The first DOCUMENT run captures exact opt-in
gates for arbitrary page JavaScript and simulated Replay trades, offline verification, native review
authority, minimal OpenCode Replay exposure, and the remaining inability to prove broker/account isolation.
**Startup:** continue the nested corpus; direct source reading and offline tests only unless a later run
receives explicit authorization for live TradingView/CDP work.

---

### 24 — web-hmi10cf `[CERT]`

**`live-install`** (lab unit 192.168.0.40, MAC 00:30:d8:0e:d5:08). Flat corpus, prefix `hmi-`.
Honeywell/CentraLine WEB-HMI10/CF 10.1" HTML5 web panel (i.MX8M Mini Cortex-A53, Linux, Chromium
kiosk) — thin client for Niagara N4 / CIPer / WEB-8000 / CentraLine / ARENA NX. Sources: 4 official
PDFs (datasheet EN0Z-1063GE51, user manual EN2Z-1077, install 31-00390, CIPer 50 guide 31-00198)
`[CERT-doc]`/`[CERT-hw]`. **STATIC STOP 8/11 gaps**; the 2 remaining live gaps are BLOCKED (device
internal firewall + WSL2-mirrored TCP prevent reachability). Live probes (§12) pending reachability.
**Startup:** continue static analysis; live gaps need a network path to the device.
...
**Document-mode corpus** (§20) capturing the architecture, API discovery, and operational runbook for extracting data from two solar-panel monitoring web portals — **Solis** (Ginlong SolisCloud) and **FlexOM** (CPS/Chint Power Systems) — using only web-portal credentials, no official API keys.

Subject artifacts: `scraper/capture.mjs`, `scraper/extract-solis.mjs`, `scraper/daemon.mjs` (Playwright Node.js extraction scripts); `scraper/data/solis/` (captured API JSON responses: `station_list.json` 121 fields, `inverter_listV2.json` 149 fields, `chart_inverter_day_v2.json` 83 pts, `inverter_detail.json` 781 fields, `_manifest.jsonl`); `scraper/data/flexom/pulled/` (11 FlexOM endpoints; Bearer auth); `scraper/logs/daemon.log`; `scraper/docs/PROCESS.md`; `scraper/refresh-loop.sh`; `scraper/dashboard/` (Cloudflare Pages delivery).

Bootstrap 2026-09-11: nested corpus under `corpus/`; git initialized; no remote yet. Hook file present but not registered (settings.json not yet wired). SECRETS DISCIPLINE applies (credentials in `../.secrets/crendeciales.env` and CF token in `scraper/.tunnel-token` — cite structure, never values).
