...
| 15 | gateway-ug67 | `/home/cristian/investigacion/gateway-ug67` | **mature** (16 md / 1 run / 1 retro / git yes / hook yes) `[CERT]` · **`live-install`** (physical device) · **COMPLETE** (serial + web-GUI + SSH + HTTP-API; firmware-RE blocked=encrypted, root-shell not-reachable) | **Live hardware**: Milesight UG67 Outdoor LoRaWAN Gateway (US915, fw 60.0.0.47, Quagga vtysh CLI) — serial console COM4 + web GUI (chrome-devtools) + official PDFs `[CERT-hw]`/`[CERT-doc]` | serial driver (`SerialPort` via WSL interop) + chrome-devtools MCP + `fetch-doc.sh` + `extract-pdf.sh`; dynamic/hardware phase §12 | English `[CERT]` |
