...
**Hook status** (2026-09-25, #1147 round 2/3): `.claude/hooks/retro-gate-stop.sh` IS registered as a Stop hook in `.claude/settings.json`, and the script exists and is executable. It has never fired in practice because no session in the retained transcript history (`~/.claude/projects`) was ever launched from this target's own root — every session touching this corpus ran from elsewhere (`niagara-research`, `tunnel/clientes/Leon-Guanajuato/Pancaddia`). Same standard as row 13 (three.js, #1134): registered ≠ firing.
...
**Hook status** (2026-09-25, #1147 round 2/3): `.claude/hooks/retro-gate-stop.sh` IS registered as a Stop hook in `.claude/settings.json`, and the script exists and is executable. It has never fired in practice because no session in the retained transcript history (`~/.claude/projects`) was ever launched from this target's own root — the corpus was created entirely inside the review window, and the work that produced it ran from `investigacion/mini-pc` and `sdd-investigacion`. Same standard as row 13 (three.js, #1134): registered ≠ firing.
