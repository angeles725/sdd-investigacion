...
|---|---|---|
| [`research-sdd-init.sh`](toolbelt/research-sdd-init.sh) | `<target-dir> [--corpus auto\|nested\|flat] [--prefix <slug>] [--force] [--wire] [--no-wire]` | Mechanical bootstrap scaffolder; refuses over an existing corpus. (`nested` = corpus in a `corpus/` subdir, `flat` = at target root, `auto` = decide.) By default (PROPOSE-NEVER-APPLY), prints the `.claude/settings.json` hook-wiring snippet for the operator to paste. Pass `--wire` to have the script write it automatically (requires `jq`); on an existing corpus `--wire` skips the scaffold entirely and instead REPAIRS any missing hook file (create-only — never overwrites an existing, possibly hand-adapted, hook) before merging settings.json; SessionStart is wired only once `research-protocol.sh` no longer carries the `<SUBJECT>` placeholder (unadapted hooks print a `WARN` and are skipped; Stop still merges). `--no-wire` is a backward-compat alias for the default (print-only, no write). |
