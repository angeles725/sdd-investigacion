...
|---|---|---|
| [`research-sdd-init.sh`](toolbelt/research-sdd-init.sh) | `<target-dir> [--corpus auto\|nested\|flat] [--prefix <slug>] [--force] [--wire] [--no-wire]` | Mechanical bootstrap scaffolder; refuses over an existing corpus. (`nested` = corpus in a `corpus/` subdir, `flat` = at target root, `auto` = decide.) By default (PROPOSE-NEVER-APPLY), prints the `.claude/settings.json` hook-wiring snippet for the operator to paste. Pass `--wire` to have the script write it automatically (requires `jq`); on an existing corpus `--wire` validates an existing settings.json as JSON first (exit 4, nothing written, if invalid), then skips the scaffold entirely and instead REPAIRS any missing hook file (create-only — never overwrites an existing, possibly hand-adapted, hook) before merging settings.json. Dedup recognises a hook already registered via `$CLAUDE_PROJECT_DIR` (unquoted/quoted/braced) as the same hook as the absolute path this script writes, and reports `already wired` rather than `registered` on a re-run. SessionStart is wired only once `research-protocol.sh` no longer carries a live (non-comment) `<SUBJECT>` placeholder (unadapted hooks print a `WARN` and are skipped; Stop still merges). `--no-wire` is a backward-compat alias for the default (print-only, no write). |
