...
|---|---|---|
| [`research-sdd-init.sh`](toolbelt/research-sdd-init.sh) | `<target-dir> [--corpus auto\|nested\|flat] [--prefix <slug>] [--force] [--wire] [--no-wire] [--scaffold]` | Mechanical bootstrap scaffolder; refuses over an existing corpus. (`nested` = corpus in a `corpus/` subdir, `flat` = at target root, `auto` = decide.) By default (PROPOSE-NEVER-APPLY), prints the `.claude/settings.json` hook-wiring snippet for the operator to paste. Pass `--wire` to have the script write it automatically (requires `jq`); on an existing corpus `--wire` validates a NON-EMPTY existing settings.json as an object with a valid `.hooks` shape first (exit 4, nothing written, if not — an empty file is treated as `{}`, not refused), then skips the scaffold entirely and instead REPAIRS any missing hook file (create-only — never overwrites an existing, possibly hand-adapted, hook) before merging settings.json (pretty-printed, so a hand-maintained file keeps its indentation; a merge failure also exits 4, never 0). Dedup recognises a hook already registered via `$CLAUDE_PROJECT_DIR` (unquoted/quoted/braced/whole-string-quoted) as the same hook as the absolute path this script writes, and reports `already wired` rather than `registered` on a re-run. SessionStart is wired only once `research-protocol.sh` exists and no longer carries a live (non-comment) `<SUBJECT>` placeholder (an absent or unadapted hook prints a `WARN` and is skipped; Stop still merges). If the target has NO corpus marker at all (`INDEX.md`, `RESEARCH-STATE.md`, the §16 multi-focus `RESEARCH-STATE-<focus>.md`, or `CATALOG.md`), `--wire` alone REFUSES (exit 5, nothing written — no dirs, no `.gitignore` edit, no `git init`, regardless of `--force`) instead of silently scaffolding: an operator asking to WIRE never intends to SCAFFOLD (kit issue #1047). Pass `--scaffold` together with `--wire` to opt in to scaffolding and wiring in one call (`--scaffold` alone, without `--wire`, is a usage error); SessionStart is still skipped there under the same live-`<SUBJECT>` guard. `--no-wire` is a backward-compat alias for the default (print-only, no write). |
