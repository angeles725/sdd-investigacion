...
# Purge residual gate state left by the staged-swap case above (it leaves gate.go/gate.ready
# behind): a stale gate.go would make the bwrap stub skip the handshake and never signal
# gate.ready, so this race case must start from a clean gate. Load-bearing, not cosmetic.
