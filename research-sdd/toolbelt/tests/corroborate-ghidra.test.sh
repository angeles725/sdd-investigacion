...
mkheadless timeout; run swapped 3 >/dev/null 2>"$ROOT/swap.err" & pid=$!; for _ in $(seq 1 300); do [ "$(stat -c %a "$ROOT/.swapped.stage/tool/ghidra/support/analyzeHeadless" 2>/dev/null)" = 500 ] && break; sleep .01; done; printf 'changed\n' >"$ROOT/target.bin"; printf 'replacement\n' >"$ROOT/gh/support/analyzeHeadless"; wait "$pid"; rc=$?
if [ "$rc" -ne 0 ] && python3 -c 'import json,sys;d=json.load(open(sys.argv[1]));assert d["input"]["source"]["sha256"]==d["input"]["staged"]["sha256"]' "$ROOT/swapped/report.json"; then ok 'source and tool swaps cannot change staged bytes'; else cat "$ROOT/swap.err"; no 'source and tool swap resistance'; fi
mkheadless timeout; run publication-race 1 >/dev/null 2>&1 & pid=$!; for _ in $(seq 1 300); do [ -e "$ROOT/.publication-race.stage/tool/ghidra/support/analyzeHeadless" ] && break; sleep .01; done; mkdir "$ROOT/publication-race"; echo keep >"$ROOT/publication-race/keep"; wait "$pid"; rc=$?
if [ "$rc" -eq 2 ] && [ "$(cat "$ROOT/publication-race/keep")" = keep ] && [ ! -e "$ROOT/.publication-race.stage" ]; then ok 'publication race replaces nothing'; else no 'publication race'; fi
