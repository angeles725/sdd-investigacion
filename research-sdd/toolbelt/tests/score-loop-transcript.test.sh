...
echo "$out" | grep -qE '^C1 degraded unrecognized transcript shape' \
  && ok "origin-less: C1 degraded (no record carries .origin)" || no "origin-less: C1 ($out)"
echo "$out" | grep -qE '^C2 degraded unrecognized transcript shape' \
  && ok "origin-less: C2 degraded" || no "origin-less: C2 ($out)"
echo "$out" | grep -qE '^C3 degraded unrecognized transcript shape' \
  && ok "origin-less: C3 degraded" || no "origin-less: C3 ($out)"
echo "$out" | grep -qE '^C4 degraded unrecognized transcript shape' \
  && ok "origin-less: C4 degraded" || no "origin-less: C4 ($out)"
echo "$out" | grep -qE '^SUMMARY pass=0 fail=0 n/a=0 degraded=4$' \
...
echo "$out" | grep -qE '^C3 pass compaction detected' \
...
out="$(RSDD_STATUS_SCRIPT="$STUB_MULTIFOCUS_MIXED" bash "$SUT" --corpus "$ROOT/continue" \
  --transcript "$FIX/continue-clean.jsonl" 2>&1)"
echo "$out" | grep -qE '^C4 fail stop_token=present status_next=STOP queue=non-empty commits_after_stop=0' \
...
echo "$out" | grep -qE '^C1 fail only 1 block commit in window' \
...
echo "$out" | grep -qE '^C1 fail 0 block commits' \
...
echo "$out" | grep -qi "WARN: implausible block range B1000-B1200" \
...
if [ "${1:-}" = "--prove-teeth" ]; then
  ...
  if grep -qF "| head -n1)\"" "$m"; then
    mo="$(RSDD_STATUS_SCRIPT="$STUB_MULTIFOCUS_MIXED" bash "$m" --corpus "$ROOT/continue" \
      --transcript "$FIX/continue-clean.jsonl" 2>&1)"
    if echo "$mo" | grep -qE '^C4 pass'; then
  ...
  if [ "$(grep -c 'UNRECOGNIZED_SHAPE=0' "$m")" -ge 3 ]; then
    mo="$(RSDD_STATUS_SCRIPT="$STUB_STOP" bash "$m" --corpus "$ROOT/continue" \
      --transcript "$FIX/origin-less.jsonl" 2>&1)"
    if echo "$mo" | grep -qE '^C1 pass 2 block commits, 0 operator turns'; then
  ...
  if grep -qF 'if [[ "$span" -gt 999999 ]]; then' "$m"; then
    ...
    if echo "$mo" | grep -qE '^C1 n/a 201 block commits'; then
  ...
  if grep -qF 'status_default="$(bash "$STATUS_SCRIPT" "$CORPUS" 2>&1)"' "$m"; then
    mo="$(RSDD_STATUS_SCRIPT="$STUB_WARN_STDERR" bash "$m" --corpus "$ROOT/continue" \
      --transcript "$FIX/continue-clean.jsonl" 2>&1)"
    if echo "$mo" | grep -qE '^C4 fail'; then
