...
# viable for the cli()-as-subprocess path. The RED-INV5 test (direct-import path)
# uses monkeypatching and does catch the N>1 case.
...
with tempfile.TemporaryDirectory() as td:
    ...
    try:
        ...
        _inv5_created_dirs = []
        _orig_mrs_inv5 = _dc_inv5.make_run_subdir
        def _tracking_mrs_inv5(run_uuid, root=_dc_inv5._DEFAULT_RSDD_ROOT):
            rd = _orig_mrs_inv5(run_uuid, root)
            _inv5_created_dirs.append(rd)
            return rd
        _dc_inv5.make_run_subdir = _tracking_mrs_inv5
        try:
            _raised_inv5 = None
            try: _de_inv5.DetonateVmExecutor(tmp / "out").evaluate(_inv5_plan)
            except Exception as _e_inv5: _raised_inv5 = _e_inv5
        finally:
            os.environ["PATH"] = _old_inv5
            _dc_inv5.make_run_subdir = _orig_mrs_inv5  # always restore
        ...
        _run_dir_inv5 = _inv5_created_dirs[0]
        assert not Path(_run_dir_inv5).exists(), (
            f"INV-5 FAIL: run_dir leaked after pre_boot GateError: {_run_dir_inv5!r}"
        )
...
# Mutation controls for the isolation fixes in RED11 and RED-INV5.
...
if PROVE_TEETH:
    ...
    with tempfile.TemporaryDirectory() as td:
        tmp = Path(td); p = _shims(tmp)
        try:
            import detonate_exec as _de_t5t; from gate import GateError as _GE_t5t
            ...
            try:
                try: _de_t5t.DetonateVmExecutor(tmp / "out").evaluate(_t5t_plan)
                except Exception: pass
            finally:
    ...
    with tempfile.TemporaryDirectory() as td:
        tmp = Path(td); p = _shims(tmp)
        try:
            import detonate_exec as _de_dbl; from gate import GateError as _GE_dbl
            ...
            try:
                try: _de_dbl.DetonateVmExecutor(tmp / "out").evaluate(_dbl_plan)
                except Exception: pass
            finally:
