...
write_stub 0 "Summary: 0 tagged breakthrough(s) · 0 unindexed · 0 drifted.
RSDD-STATE: clean"
OUT="$(bash "$TMP/sweep-breakthroughs-hook.sh" 2>&1)"; RC=$?
[ -z "$OUT" ] \
...
write_stub 0 "WARN: unindexed breakthrough.
...
if [ "${1:-}" = "--prove-teeth" ]; then
  ...
  if ! grep -qF '= "clean"' "$SUT"; then
    ...
    write_stub 0 "Summary: 0 tagged · 0 unindexed · 0 drifted.
RSDD-STATE: clean"
    cp "$TMP/mutant-hook-ha.sh" "$TMP/sweep-breakthroughs-hook.sh"
    MUTANT_OUT_HA="$(bash "$TMP/sweep-breakthroughs-hook.sh" 2>&1)"
  ...
  if ! grep -qF '${state:-}' "$SUT"; then
    ...
    write_stub 0 "Summary: 0 tagged breakthrough(s)."
    cp "$TMP/mutant-hook-hb.sh" "$TMP/sweep-breakthroughs-hook.sh"
    MUTANT_OUT_HB="$(bash "$TMP/sweep-breakthroughs-hook.sh" 2>&1)"
  ...
  cp "$TMP/mutant-hook-hc.sh" "$TMP/sweep-breakthroughs-hook.sh"
  MUTANT_OUT_HC="$(bash "$TMP/sweep-breakthroughs-hook.sh" 2>&1)"
