...
| `score-loop-transcript.sh --corpus <dir> [--transcript <jsonl>] [--base-ref <ref>] [--since <date>] [--until <date>]` | Eval scorer for a loop run (kit issue #993, WU5): C1 continued past block 1 (>=2 block commits in the window with no operator input between them — commit count alone without `--transcript`), C2 questions asked (assistant final-message last line matching a question pattern), C3 compaction survived (block commits before/after the first compaction event in the transcript), C4 STOP honored (RETURN CONTRACT `STOP:` token in the final return + `research-sdd-status.sh` confirms 0 open/empty queue + no block commit after STOP). Backs `evals/profile-ab/PROTOCOL.md`'s Claude-vs-Qwen, claude-vs-general-profile decision rule. One `C<n> pass\|fail\|n/a\|degraded <evidence>` line per criterion + a `SUMMARY` line; §7 anti-silent-zero: git and jq are probed and report typed `degraded` on absence; n/a (no transcript given) is always distinct from degraded (transcript given but unusable) and from no-match (transcript usable, nothing found — e.g. no compaction event). Overridable via `RSDD_BLOCK_COMMIT_REGEX`, `RSDD_OPERATOR_INPUT_JQ`, `RSDD_COMPACT_JQ`, `RSDD_STOP_TOKEN_REGEX`, `RSDD_STATUS_SCRIPT` (see script header — the Claude Code JSONL "operator input"/"compaction" record shape is a documented, unverified-against-a-real-harness-build heuristic, not a confirmed schema). Never modifies the corpus. Exit 0 = scored (individual criteria may still be fail/n-a/degraded); 1 = corpus missing/not a git repo; 2 = bad args; 3 = degraded (git unavailable). |
