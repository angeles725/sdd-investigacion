...
| `research-sdd-init.sh <target-dir> [--corpus auto\|nested\|flat] [--prefix <slug>] [--force] [--no-wire]` | Mechanical BOOTSTRAP scaffolder: creates corpus skeleton, copies templates, git-inits, rolls back on failure (METHODOLOGY BOOTSTRAP). Refuses if a corpus already exists. By default auto-wires Stop and SessionStart hook entries into `$target/.claude/settings.json` using jq (idempotent merge, preserves existing hooks). `--no-wire` skips the write and prints the JSON block for manual pasting instead. If jq is absent, degrades gracefully: emits a `degraded:` message to stderr and falls back to print-only. Emits follow-up checklist for the judgment half (classify artifact, seed gaps, register target). Exit 0 = scaffolded; 1 = post-scaffold artifact missing (internal failure); 2 = bad args/not writable/missing template; 3 = corpus already exists (refused). |
