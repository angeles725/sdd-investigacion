...
state="$(find "$target" -maxdepth 3 -name 'RESEARCH-STATE*.md' -not -path '*/.git/*' 2>/dev/null | head -1)"
[ -f "$state" ] || { echo "verify-state: no RESEARCH-STATE*.md under $target" >&2; exit 2; }

echo "== verify-state: $(basename "$state") (target: $target) =="
rc=0

# 1. backlog `pending` gap rows
pending="$(grep -icE '\bpending\b' "$state" 2>/dev/null || echo 0)"
...
# 3. covered-blocks claim vs on-disk block files (best-effort glob, corpus-relative)
covered_claim="$(grep -iE 'covered blocks' "$state" 2>/dev/null | grep -oE '[0-9]+' | head -1)"
ondisk="$(find "$(dirname "$state")" -maxdepth 1 -name '*block*.md' 2>/dev/null | wc -l | tr -d ' ')"
...
[ "$rc" -eq 0 ] && echo "   ok     summary is consistent with the backlog."
echo "== exit $rc =="
exit $rc
