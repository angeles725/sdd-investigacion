...
for t in INDEX.template.md RESEARCH-STATE.template.md SOURCES.template.md hook-sessionstart.sh; do
...
# so tools/ is no longer scaffolded. (`tools` stays in is_inproject's ignore list for legacy targets.)
mk  "$corpus/sources"; mk "$target/.claude/hooks"; mk "$target/retros"
...
for f in "$corpus/INDEX.md" "$corpus/RESEARCH-STATE.md" "$corpus/sources/SOURCES.md" \
         "$target/.claude/hooks/research-protocol.sh"; do
...
echo "  target : $target"
echo "  corpus : ${rel}"
echo "  created: INDEX.md · RESEARCH-STATE.md · sources/SOURCES.md · hook · retros/ · .gitignore"
