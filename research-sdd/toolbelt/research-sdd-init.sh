...
for t in INDEX.template.md RESEARCH-STATE.template.md SOURCES.template.md gen-catalog.py hook-sessionstart.sh; do
...
mk  "$corpus/tools"; mk "$corpus/sources"; mk "$target/.claude/hooks"; mk "$target/retros"
...
cpf "$TPL/gen-catalog.py"             "$corpus/tools/gen-catalog.py"
...
for f in "$corpus/INDEX.md" "$corpus/RESEARCH-STATE.md" "$corpus/sources/SOURCES.md" \
         "$corpus/tools/gen-catalog.py" "$target/.claude/hooks/research-protocol.sh"; do
...
echo "  target : $target"
echo "  corpus : ${rel}"
echo "  created: INDEX.md · RESEARCH-STATE.md · sources/SOURCES.md · tools/gen-catalog.py · hook · retros/ · .gitignore"
