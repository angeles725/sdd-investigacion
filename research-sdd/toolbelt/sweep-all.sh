#!/usr/bin/env bash
# sweep-all.sh — aggregator: run all six canonical Research-SDD session-start sweep scripts
...
# run manually. This shim collapses six commands into one, raising compliance probability.
# Claude and OpenCode run the same six scripts automatically via their own hooks/plugins — this
...
# aborts the remaining scripts. All six always run. Exit is non-zero if ANY failed.
#
...
#   - All six underlying scripts are read-only audits; sweep-all.sh never mutates anything.
...
# Exit : 0 all six scripts passed · non-zero at least one script failed or timed out
