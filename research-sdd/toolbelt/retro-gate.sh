...
_run_issue_seeding() {
  ...
  local created=0 skipped=0 failed=0 ran=0
  ...
  printf 'retro-gate: issue-seeding: ran=%d created=%d skipped-dedup=%d failed=%d target=%s\n' \
    "$ran" "$created" "$skipped" "$failed" "$(basename "$target")" >&2
}
