...
4. **Run the loop.** Execute the NORMAL CYCLE one iteration = one cited block, and self-continue per the
   ...
   (`toolbelt/retro-gate.sh`, kit issue #479) blocks the session once until it holds. The Stop hook (blocking + delta
   auto-seeding) is Claude-Code-only (kit issue #1110): the kit wires no Stop-equivalent for codex or reasonix, so
   their runs get neither the block nor the auto-seeding. On codex/reasonix, run
   `$KIT/toolbelt/stage-retro-issues.sh <retro> --apply` by hand once the retro is written — see PROMPT-LOOP's RETRO
   CHECKPOINT for the exact point and why.
