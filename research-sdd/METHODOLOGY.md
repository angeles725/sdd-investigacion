...
<path>` or `retro: not-due` in its final return (PROMPT-LOOP RETRO CHECKPOINT). **Instrument status:** kit issue
#479 adds `toolbelt/verify-retro.sh` (typed conformance check of one retro) and `toolbelt/retro-gate.sh`, the
target's Stop hook: it allows when nothing research-related changed, blocks ONCE with the exact missing element
otherwise, and never blocks twice in a session (a gate that can block forever is a trap). `research-sdd-init.sh`
prints the `.claude/settings.json` wiring; the operator applies it per target (propose-never-apply). Until #479
lands, the PROMPT-LOOP exit condition is the only gate.
