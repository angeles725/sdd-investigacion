...
|---|---|---|---|---|---|
| T1 | colaborador's session-local scratchpad flip-prover + runtime harnesses (uncommitted — NOT in the kit tree; they vanish with the session): assembled pre-fix SUT + new test files, ran them, required each named gate test RED on pre-fix (`acid-prefix-sut.sh` and `gate-findings-aggregation.sh` are ephemeral session scripts; a reader who greps the repo finds nothing) | — | — | colaborador's scratchpad harness (uncommitted, session-local) · SEEs whether a named test actually fails on old code rather than recomputing the logic under test; the DURABLE artifact is the PROCEDURE ("run new/changed tests against the pre-fix SUT; require each gate test to flip red→green"), captured as proposed delta D2 (executed-red-before-fix) | `D2-durable` · the procedure is the durable artifact (proposed delta D2: executed-red-before-fix); flip-prover first iteration keyed on keyword "ABSENT" (false positive on test 25, false negative on test 60) — re-cut to enumerate gate-test numbers per suite (#7771); a COMMITTED reusable prover (`*.sh` + `*.test.sh` under `toolbelt/`) is a SEPARATE future kit work unit that does NOT exist yet — do not treat these scratchpad scripts as committed kit tools |
