...
| Test suite | `bash research-sdd/toolbelt/tests/run-all.sh` | One gate command traverses BOTH corpora: `research-sdd/toolbelt/tests/*.test.sh` and `research-sdd/install/tests/*.test.sh` (kit issue #1126 — CI already ran the install corpus, this local gate did not, so a PR could be green locally and still fail CI). The aggregate declares each corpus and its suite count as a separate line (§7) before the merged run. All suites pass; skipped ≠ passed; zero-coverage run exits 1 |
