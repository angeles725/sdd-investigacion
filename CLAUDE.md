...
|---|---|---|
| Shellcheck | `shopt -s globstar && shellcheck -S warning research-sdd/toolbelt/**/*.sh research-sdd/install/**/*.sh` | Zero warnings. **`globstar` is required** — without it the glob matches depth 1 only, silently skipping all of `lib/` and `tests/`. `shopt` is a bash builtin: under zsh it errors out, but zsh globs `**` recursively on its own, so the coverage is still correct — check the file count, not the absence of an error. `.shellcheckrc` suppresses SC2015 and SC2016 — intentional silences for this codebase's idioms, not blanket ignores. `research-sdd/install/**/*.sh` is a second, separate tree (`adapters.sh`, `install.sh`, `research-sdd-install.sh`, `tests/research-sdd-install.test.sh`) outside `research-sdd/toolbelt/` — the toolbelt glob alone does not cover it (kit issue #1126) |
| Test suite | `bash research-sdd/toolbelt/tests/run-all.sh` | One gate command traverses BOTH corpora: `research-sdd/toolbelt/tests/*.test.sh` and `research-sdd/install/tests/*.test.sh` (kit issue #1126 — CI already ran the install corpus, this local gate did not, so a PR could be green locally and still fail CI). The final AGGREGATE RESULT block declares each corpus and its suite count as its own line (§7) — after all suites have run, not before. All suites pass; skipped ≠ passed; zero-coverage run exits 1; an ABSENT install/tests corpus exits 1 even if every discovered suite passes (kit issue #1144 — a moved/renamed corpus must not read as a silent pass; an EMPTY install/tests dir is reported distinctly and does not fail the gate by itself) |
